defmodule Elixirfm.Artist do
  ...
  def search(query, args \\ [page: 1, limit: 30]) do
    Elixirfm.get_request('artist.search&artist=#{query}&page=#{args[:page]}&limit=#{args[:limit]}')
  end
  ...
  def get_info(query, args \\ [lang: "eng", username: ""]) do
    Elixirfm.get_request("artist.getinfo&artist=#{query}&lang=#{args[:lang]}&username=#{args[:username]}")
  end
  ...
  def get_similar(query, args \\ [limit: 10]) do
    Elixirfm.get_request("artist.getsimilar&artist=#{query}&limit=#{args[:limit]}")
  end
  ...
  #   Elixirfm.get_request("artist.getTags&artist=#{query}")
  # end
  ...
  def get_top_album(query, args \\ [page: 1, limit: 30]) do
    Elixirfm.get_request("artist.gettopalbums&artist=#{query}&page=#{args[:page]}&limit=#{args[:limit]}")
  end
  ...
  def get_top_tags(query) do
    Elixirfm.get_request("artist.gettoptags&artist=#{query}")
  end
  ...
  def get_top_tracks(query, args \\ [page: 1, limit: 15]) do
    Elixirfm.get_request("artist.gettoptracks&artist=#{query}&page=#{args[:page]}&limit=#{args[:limit]}")
  end
  ...
  def get_correction(query) do
    Elixirfm.get_request("artist.getcorrection&artist=#{query}")
  end
